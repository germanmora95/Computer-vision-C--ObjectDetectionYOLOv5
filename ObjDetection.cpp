...
void draw_label(Mat img, string label, int left, int top){ // This takes the image, the label text, and the TL points (left,x) and (top, y) coordinates.
    ...
    Point Point_BR = Point(left+label_size.width,top+label_size.height); // PLAY WITH BASELINE. 
...
Mat Postprocessing(Mat img, vector<Mat> results, const vector<string> class_names){
    ...
    cout << "Bounding boxes detected: " << Boundingbox.size() << endl; 
    ...
    cout << "Amount of filtered bounding boxes: " << indices.size() << endl;                                                                                // Indices is a vector of the bounding boxes IDs that survive the non-maximum suppression algorithm. NMS_THRESHOLD: anything above this value in IoU is considered overlapped Bounding box.
...
int main()
{
   ...
   string line;
   string path = "/Users/german/Library/CloudStorage/Dropbox/Projects/CV_Cpp_ObjDetection_YOLOv5/YOLOv5/data/images/bus.jpg";
   ...
   results = Preprocessing(img, network);

   img_processed = Postprocessing(img.clone(), results, class_list);
